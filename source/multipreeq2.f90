...
!
  ...
  integer   :: ppiinit    ! initial proton particle number
  integer   :: pnuinit    ! initial neutron particle number
...
!
  ...
  do ipp = 0, maxpar
    do ipn = 0, maxpar
      sumfeed = sumfeed + xspopph2(Zcomp, Ncomp, nex, ipp, ipn)
    enddo
  enddo
...
!
  do ipp = 0, maxpar
    ihp = ipp + Zcomp - ppiinit
    if (ihp < 0 .or. ihp > maxpar) cycle
    do ipn = 0, maxpar
      ihn = ipn + Ncomp - pnuinit
      if (ihn < 0 .or. ihn > maxpar) cycle
      ip = ipp + ipn
      if (ip == 0 .or. ip > maxpar) cycle
      ih = ihp + ihn
      if (ih == 0 .or. ih > maxpar) cycle
      feedph = xspopph2(Zcomp, Ncomp, nex, ipp, ipn)
...
!
      do type = 1, 2
        ...
        do nexout = Nlast(Zix, Nix, 0) + 1, nexmax(type)
          if (mpreeqmode == 2) then
            xspopph2(Zix, Nix, nexout, ipp - zejec, ipn - nejec) = &
              xspopph2(Zix, Nix, nexout, ipp - zejec, ipn - nejec) + term(type, nexout)
          else
            do ppi = ppi0, maxpar
              if (ppi - zejec < 0) cycle
              hpi = hpi0 + ppi - ppi0
    ...
    hnu = hnu0 + pnu - pnu0
                ...
                h = hpi + hnu
                if (h > maxpar) cycle
                xspopph2(Zix, Nix, nexout, ppi - zejec, pnu - nejec) &
                  = xspopph2(Zix, Nix, nexout, ppi - zejec, pnu - nejec) + factor(type, nexout, ppi, pnu)
              enddo
            enddo
          endif
...
!
      if (mpreeqmode == 2) then
        if (ip <= maxpar - 1 .and. ih <= maxpar - 1) then
          if (ipp <= maxpar - 1 .and. ihp <= maxpar - 1) xspopph2(Zcomp, Ncomp, nex, ipp + 1, ipn) = &
            xspopph2(Zcomp, Ncomp, nex, ipp + 1, ipn) + 0.5 * (feedph - sumph)
          if (ipn <= maxpar - 1 .and. ihn <= maxpar - 1) xspopph2(Zcomp, Ncomp, nex, ipp, ipn + 1) = &
            xspopph2(Zcomp, Ncomp, nex, ipp, ipn + 1) + 0.5 * (feedph - sumph)
        endif
      endif
    enddo
  enddo
